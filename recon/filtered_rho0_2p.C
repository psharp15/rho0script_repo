...
void filtered_rho0_2p(const char *inputfilename, const char *inputTreename,
  ...
  TTree *outputTree =
      new TTree("filtered_helium_2p_inc", "filtered_helium_2p_inc");
  ...
  TH1D *h_zprot_lead_vertex = new TH1D(
      "prot lead Zvertex", "protlead Zvertex;Vertex z [cm];Counts", 50, 0, 100);
  hist_list.push_back(h_zprot2vertex);
  ...
  TH3D *h_prot_duplicate_tracks_presort =
      new TH3D("prot_duplicate_tracks_presort",
               "prot_duplicate_tracks PRE "
               "SORTING;p1.pX-p2.X[GeV];p1.pY-p2.pY[GeV];p1.pZ-p2.pZ[GeV]",
               100, -2, 2, 100, -2, 2, 100, -2, 2);
  hist_list.push_back(h_prot_duplicate_tracks_presort);
  TH3D *h_prot_duplicate_tracks_uncut = new TH3D(
      "prot_duplicate_tracks_uncut",
      "prot_duplicate_tracks;p1.pX-p2.X[GeV];p1.pY-p2.pY[GeV];p1.pZ-p2.pZ[GeV]",
      50, -2, 2, 50, -2, 2, 50, -2, 2);
  hist_list.push_back(h_prot_duplicate_tracks_uncut);
  TH3D *h_prot_duplicate_tracks_protmisrecon =
      new TH3D("prot_duplicate_tracks_protmisrecon",
               "prot_duplicate_tracks "
               "CUT;p1.pX-p2.X[GeV];p1.pY-p2.pY[GeV];p1.pZ-p2.pZ[GeV]",
               100, -2, 2, 100, -2, 2, 100, -2, 2);
  ...
  TH1D *h_rho0counts_pre_prot_sorting =
      new TH1D("Rho0_Counts_PRE_prot_sorting",
               "Rho0Counts_PRE_prot_sorting;Mass [GeV];Counts", 90, 0, 5);
  ...
  // TH1D *h_energy_balance = new TH1D ("energy_balance","Energy Balance; Final
  // energy- initial energy ;counts",50,-10,10);
  // hist_list.push_back(h_energy_balance);
  ...
  for (int event = 0; event < inputTree->GetEntries(); event++) {
    ...
    h_prot_duplicate_tracks_presort->Fill(p4_prot1_kin->X() - p4_prot2_kin->X(),
                                          p4_prot1_kin->Y() - p4_prot2_kin->Y(),
                                          p4_prot1_kin->Z() -
                                              p4_prot2_kin->Z());
    ...
    TLorentzVector p4_pmiss =
        (*p4_prot_lead_kin + (*p4_pip_kin + *p4_pim_kin) - *p4_beam_kin);
    ...
    TLorentzVector p4_pmiss_thrown =
        (*p4_prot_lead_thrown + (*p4_pip_thrown + *p4_pim_thrown) -
    ...
    TLorentzVector delta4mom =
        (*p4_pip_kin + *p4_pim_kin + *p4_prot1_kin + *p4_prot2_kin -
    ...
    else if (p4_prot1_kin->P() < pleadcutoff &&
             p4_prot2_kin->P() > pleadcutoff) {
    ...
    h_coplanarity->Fill(fabs(v3_rho.Phi() - v3_prot_lead.Phi()) * 180 /
                        TMath::Pi());
    h_pip_p_theta_uncut->Fill(180. / M_PI * p4_pip_meas->Theta(), v3_pip.Mag());
    h_prot_p_theta_uncut->Fill(180. / M_PI * p4_prot_lead_kin->Theta(),
                               v3_prot_lead.Mag());
    h_prot_duplicate_tracks_uncut->Fill(p4_prot1_kin->X() - p4_prot2_kin->X(),
                                        p4_prot1_kin->Y() - p4_prot2_kin->Y(),
                                        p4_prot1_kin->Z() - p4_prot2_kin->Z());

    h_prot_p_theta_THROWN_uncut->Fill(180. / M_PI * v3_prot_lead_thrown.Theta(),
                                      v3_prot_lead_thrown.Mag());
    ...
    h_prot_p_theta_THROWN_eb->Fill(180. / M_PI * v3_prot_lead_thrown.Theta(),
                                   v3_prot_lead_thrown.Mag());
    h_pip_p_theta_THROWN_eb->Fill(180. / M_PI * v3_pip_thrown.Theta(),
                                  v3_pip_thrown.Mag());

    /********************************************** Cutting on Duplicate Track
     * Protons **********************************************/
    if ((fabs(p4_prot1_kin->X() - p4_prot2_kin->X()) == 0.01) &&
        (fabs(p4_prot1_kin->Y() - p4_prot2_kin->Y()) == 0.01) &&
        (fabs(p4_prot1_kin->Z() - p4_prot2_kin->Z()) == 0.01))
      continue;
    /********************************************** Fill Duplicate Track
     * Protons, Missed PID, and T - Cut Histograms *********************/
    h_prot_duplicate_tracks_protmisrecon->Fill(
        p4_prot1_kin->X() - p4_prot2_kin->X(),
        p4_prot1_kin->Y() - p4_prot2_kin->Y(),
        p4_prot1_kin->Z() - p4_prot2_kin->Z());
