...
Bool_t DS_2p_v2_fixed::Process(Long64_t locEntry) {
  ...
  for (UInt_t loc_i = 0; loc_i < Get_NumCombos(); ++loc_i) {
    // cerr << "I'm in locEntry: " << locEntry << " with this combo: " <<loc_i
    // << endl; INITIALIZE THE COMBO
    ...
    Int_t locNumOutOfTimeBunchesInTree =
        4; // YOU need to specify this number (on a single side, so that total
           // number out-of-time bunches accepted is 2 times this number for
           // left + right bunches)
    Bool_t locSkipNearestOutOfTimeBunch =
        true; // True: skip events from nearest out-of-time bunch on either side
              // (recommended).
    Int_t locNumOutOfTimeBunchesToUse = locSkipNearestOutOfTimeBunch
                                            ? locNumOutOfTimeBunchesInTree - 1
                                            : locNumOutOfTimeBunchesInTree;
    Double_t locAccidentalScalingFactor =
        dAnalysisUtilities.Get_AccidentalScalingFactor(
            Get_RunNumber(), locBeamP4.E(),
            dIsMC); // Ideal value would be 1, but deviations require added
                    // factor, which is different for data and MC.
    Double_t locAccidentalScalingFactorError =
        dAnalysisUtilities.Get_AccidentalScalingFactorError(
            Get_RunNumber(),
            locBeamP4.E()); // Ideal value would be 1, but deviations observed,
                            // need added factor.
    Double_t locHistAccidWeightFactor =
        locRelBeamBucket == 0
            ...
            : -locAccidentalScalingFactor /
                  (2 *
                   locNumOutOfTimeBunchesToUse); // Weight by 1 for in-time
                                                 // events,
                                                 // ScalingFactor*(1/NBunches)
                                                 // for out-of-time
    if (locSkipNearestOutOfTimeBunch && abs(locRelBeamBucket) == 1) {
      dComboWrapper->Set_IsComboCut(true);
      continue;
    } // Skip nearest out-of-time bunch: tails of in-time distribution also leak
      // in

    // Combine 4-vectors
    TLorentzVector locMissingP4_Measured = locBeamP4_Measured + dTargetP4;
    locMissingP4_Measured -= locPiPlusP4_Measured + locPiMinusP4_Measured +
                             locProton1P4_Measured + locProton2P4_Measured;
    ...
    if (locUsedSoFar_BeamEnergy.find(locBeamID) ==
      ...
      // dHist_BeamEnergy->Fill(locBeamP4.E()); // Fills in-time and out-of-time
      // beam photon combos
      dHist_BeamEnergy->Fill(
          locBeamP4.E(), locHistAccidWeightFactor); // Alternate version with
    ...
    if (locUsedSoFar_MissingMass.find(locUsedThisCombo_MissingMass) ==
      ...
      // particles dHist_MissingMassSquared->Fill(locMissingMassSquared); //
      // Fills in-time and out-of-time beam photon combos
      dHist_MissingMassSquared->Fill(
          locMissingMassSquared,
          locHistAccidWeightFactor); // Alternate version with accidental
                                     // subtraction
...
cerr << "sorting protons  " << loc_i <<"\n ";
 ...
 cerr << "sorting protons  " << tmp.T() <<"\n ";
    ...
    dFlatTreeInterface->Fill_TObject<TLorentzVector>("thrownBeam",
                                                     locBeamP4_Thrown);
    dFlatTreeInterface->Fill_TObject<TLorentzVector>("thrownPiPlus",
                                                     locMyThrownPiPlus);
    dFlatTreeInterface->Fill_TObject<TLorentzVector>("thrownPiMinus",
                                                     locMyThrownPiMinus);
    dFlatTreeInterface->Fill_TObject<TLorentzVector>("thrownProton1",
                                                     locMyThrownProton1);
    dFlatTreeInterface->Fill_TObject<TLorentzVector>("thrownProton2",
                                                     locMyThrownProton2);
